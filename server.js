...
// Middleware: Titanic Cache-Busting
...
app.get(['/', '/settlement.html'], async (req, res) => {
    try {
        ...
        // JUGGERNAUT_TITANIC_V1 Metrics
        const totalLiquidity = (assets || []).reduce((sum, a) => sum + (Number(a.gross_arbitrage_spread) || 0), 0).toLocaleString();
        const totalTimeSaved = (assets || []).reduce((sum, a) => {
            const daysToClose = a.days_to_close || 15;
            return sum + (30 - daysToClose);
        }, 0);

        const cards = (assets || []).map(a => {
            // PARTNER MERITOCRACY: Velocity Score
            const partnerVelocity = a.partner_velocity || 8.5; // Days to close
            const isBottleneck = partnerVelocity > 20;
            const velocityColor = isBottleneck ? '#ff4444' : partnerVelocity < 10 ? '#00ff00' : '#ffd700';

            // ANTI-STALL TRIGGER: Monitoring Silence
            const lastActivity = new Date(a.updated_at || a.created_at);
            const now = new Date();
            const hoursSilent = Math.floor((now - lastActivity) / (1000 * 60 * 60));
            const stallAlert = hoursSilent >= 6;

            // CASINO LIGHTS: Settlement Pulse
            const isSettling = (a.contract_status === 'Signed' && a.deal_health_score >= 90);
            const cardClass = isSettling ? 'card settling' : 'card';
            if (stallAlert) cardClass += ' stall-alert';

            return `
            <div class="${cardClass}" onclick="openModal(${JSON.stringify(a).replace(/"/g, '&quot;')})">
                <div class="velocity-tag" style="color: ${velocityColor}">
                    ${isBottleneck ? '⚠️ BOTTLENECK PARTNER' : '⚡ HIGH-VELOCITY PARTNER'} (${partnerVelocity}d)
                </div>
                <div class="header">
                    <span class="name">${a.address || 'Unknown Asset'}</span>
                    <span class="stall-badge" style="display: ${stallAlert ? 'block' : 'none'}">ANTI-STALL ACTIVE</span>
                </div>
        ...
        res.send(`<!DOCTYPE html><html><head>
            ...
            <style>
                ...
                .card { 
                    ...
                    margin-bottom: 15px; border-radius: 12px; transition: all 0.3s;
                ...
                <div class="metrics-bar">
                    ...
                    <span>📈 TIME SAVED: ${totalTimeSaved} DAYS</span>
                </div>
                ${cards}
...
app.listen(PORT, () => console.log('Juggernaut Titanic V1 Active.'));
