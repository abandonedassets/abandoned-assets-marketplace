...
BEGIN
  ...
  -- Tier-1 parity: auto-correct instead of blocking the deal (zero-friction).
  IF v_allowed
    ...
    VALUES
      (NEW.id, OLD.status, NEW.status,
       'Parity auto-correct: Tier-1 fee rerouted to Master',
       jsonb_build_object('prior_owner', NEW.active_owner, 'fee', NEW.optimized_acquisition_premium));
    NEW.active_owner := 'Master';
  END IF;
