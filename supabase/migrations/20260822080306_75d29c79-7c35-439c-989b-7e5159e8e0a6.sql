...
ALTER TABLE public.buyer_buy_boxes
  ...
  ADD COLUMN IF NOT EXISTS m2m_api_key text;

ALTER TABLE public.closing_pipeline_items
  ADD COLUMN IF NOT EXISTS m2m_box_id uuid,
  ADD COLUMN IF NOT EXISTS m2m_dispatched_at timestamptz,
  ADD COLUMN IF NOT EXISTS m2m_expires_at timestamptz,
  ADD COLUMN IF NOT EXISTS dynamic_memo_id text;
...
BEGIN
  ...
  IF COALESCE(r.tif_state,'') = 'Executed'
     OR r.status IN ('WIRE_PENDING_VERIFICATION','SETTLED_PAID') THEN
...
BEGIN
  ...
  v_memo := 'BV-' || upper(substr(replace(_id::text, '-', ''), 1, 8)) || '-'
            || to_char(now(), 'YYMMDDHH24MISS');

  UPDATE public.closing_pipeline_items
     SET status = 'WIRE_PENDING_VERIFICATION',
         tif_state = 'Executed',
  ...
  RETURN jsonb_build_object(
    ...
    'memo_id', v_memo,
    'assignment_fee', r.optimized_acquisition_premium,
    'price', r.base_contract_price
  );
END;
$$;
